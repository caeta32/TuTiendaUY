...
$productos = DB::table('productos')->orderBy('created_at', 'asc')->get();
...
<div>
    ...
    <ul class="list-group list-group-flush" style=" margin-left: 1%; width: 11%; margin-top: 2%; position: absolute"> <a href="{{ url('proddesc') }}" class="list-group-item list-group-item-action">Mayor Precio</a> <a href="{{ url('prodasc') }}" class="list-group-item list-group-item-action">Menor Precio</a> <a href="{{ url('proddescfec') }}" class="list-group-item list-group-item-action">Mas Reciente</a> <a href="{{ url('prodascfec') }}" class="list-group-item list-group-item-action">Menos Reciente</a> </ul>
</div>
<div>
        ...
        <div class="col-md-3 col-sm-6">
            <div class="product-grid3" style="background-color: #FFFFFF;border-radius: 10px; box-shadow:0px 0px 15px #777777; margin-bottom: 50px">
                <div class="product-image3" style="max-height: 600px; max-width: 600px">
                    <?php if($mail == "administradores@tutienda.com") {
                    ?>
                    <form id="formid" action="{{ route('verGlobalIniController') }}" method="POST">
                        <?php ;
                        } else {
                        ?>
                        <form id="formid" action="{{ route('verDesdeInicioController') }}" method="POST">
                            <?php ;
                            }?>
                            ...
                            <ul class="social">
                                ...
                                </button> {{-- Agregar al carrito --}} {{-- Dispara un Modal --}}
                                <li>
                                    <a role="button" data-bs-toggle="modal" data-bs-target="#addToCart" data-bs-codigo="{{ $producto->codigo }}" data-bs-nombre="{{ $producto->nombre }}" data-bs-precio="{{ $producto->precio }}" data-bs-rutaImagen="{{ $producto->rutaImagen }}" data-bs-cantidadDisponible="{{ $producto->cantidadDisponible }}"> <i class="fa fa-shopping-cart"></i> </a>
                                </li> {{-- FIN Agregar al carrito --}} </ul>
                        </form>
                </div>
...
<hr>
<div style="text-align: center; margin-top: 4%;"> <small>Copyright &copy; 2021 TuTienda Uruguay, Inc. Todos los derechos reservados.</small> </div> {{-- ================================================================= --}} {{-- MODAL --}}
        ...
        <div class="modal-content">
            ...
            <form action="{{ route('cart.add') }}" method="POST"> @csrf
                <div class="modal-body">
                    <p><strong>¡Estás a punto de agregar este producto al carrito!</strong></p> {{-- ==================================================== --}} {{-- Primer campo del form --}}
                    <label for="quantity">Indique la cantidad que desea agregar: </label>
                    <input type="number" min="1" max="{{--Se settea en el script js--}}" id="quantity" name="quantity" required> {{-- ==================================================== --}} </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-primary" data-bs-dismiss="modal">Cancelar</button> {{-- ==================================================== --}} {{-- Los otros campos del form y el submit correspondiente --}}
                    ...
                    <button type="submit" class="btn btn-primary" class="tooltip-test" title="add to cart"> <i class="fa fa-shopping-cart"></i> Añadir al Carrito </button> {{-- ==================================================== --}} </div>
            </form>
        </div>
    </div>
